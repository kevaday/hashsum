...
def train_model(model, dataloaders, criterion, optimizer, device, save_dir, model_name, num_epochs=25):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val':
                ...
                print('Saving model')
                save_model(save_dir, model, model_name, epoch_loss, epoch_acc.item())
...
def main(data_dir, save_dir='./save', model_name='densenet', checkpoint=None, num_classes=27, batch_size=32,
         num_epochs=20, learn_rate=0.001, momentum=0.9, feature_extract=False, use_pretrained=False, graph=True):
    # Initialize the model for this run
    if checkpoint:
        model_ft = load_model(checkpoint)
        feature_extract = model_ft.feature_extract
        use_pretrained = model_ft.pretrained
        input_size = 224
    else:
        model_ft, input_size = initialize_model(model_name, num_classes, feature_extract, use_pretrained=use_pretrained)
        model_ft.feature_extract = feature_extract
        model_ft.pretrained = use_pretrained
    ...
    # Train and evaluate
    model_ft, hist = train_model(model_ft, dataloaders_dict, criterion, optimizer_ft, device, save_dir, model_name,
...
if __name__ == '__main__':
    ...
    data_dir = './malevis_train_val_224x224'
    save_dir = './save'
    model_name = 'densenet'
    batch_size = 256
    checkpoint = None  # './save/densenet_loss_0.626_acc_0.ckpt'
    num_epochs = 20
    learn_rate = 0.001
    ...
    main(data_dir, save_dir, batch_size=batch_size, checkpoint=checkpoint, num_epochs=num_epochs, learn_rate=learn_rate,
         feature_extract=feature_extract, use_pretrained=use_pretrained, graph=graph)
